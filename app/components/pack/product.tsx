    ...
    return (
        <div id="products" className="flex flex-col items-center justify-center my-14 w-full gap-4 text-neutral-200">
            <div className="text-center ">
                <h1 className="text-3xl sm:4xl font-bold border-b-2 border-[#64CCC5]"><span className="text-[#64CCC5]">&</span>PRODUCT</h1>
            ...
            viewport={{ once: true}}
            className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 w-[90%] my-2 justify-center items-center transition-all duration-700">
            {PRODUCT.sort(( a, b ) => {
                ...
                (index >= showmore ? (
                    ...
                    <div className="bg-[#0b2e3b] bg-opacity-50 backdrop-blur-sm p-1 rounded-md bg-cover shadow-lg" key={index}>
                    ...
                    <div className="px-2 flex flex-row justify-between gap-2 items-center">
                        <div className="w-2/3 flex flex-row gap-2">
                            {project.tech.map((timg, i)=>(
                                <Image key={i} src={timg.image} alt={timg.name} width={70} height={50} className="w-6 h-6 rounded-md object-fill brightness-200" />
                            ))}
                        </div>
            ...
            <Button variant={"outline"} className="hover:bg-[#64CCC5] hover:text-gray-900 border-[#64CCC5]" onClick={()=>{(showmore > 3 ?  setShowmore(3) : setShowmore(PRODUCT.length))}}>{showmore > 3 ?  "Show Less" : "Show More"}</Button>
        </div>
    )
}